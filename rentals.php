...
<html>
<head>
<title>EagleVideo Rentals</title>
...
<p class="emphasis"> Rentals </p>
<p class="emphasis" style="font-size:18px;"> Movies Currently Checked Out Under Your Account</P>

<table border='2' cellpadding='3' style="padding-top:10px;"><tr>
<th style="padding:3px;">Movie Name</th>
<th style="padding:3px;">Date Taken Out</th>
<th style="padding:3px;">Return Movies</th> </tr>
...
$custid = $row5["memberid"];
	...
	$query5 = "Select m.title, r.movieid, r.dateout From rentals r, movie m Where r.movieid = m.mid and r.memberid =" .$custid . "and r.datein is null";
$finalresult = pg_query($dbconn, $query5) or die("Query5 Failed: " . pg_last_error());
while ($row = pg_fetch_array($finalresult)){
	$movieid = $row["movieid"];
	$title = $row["title"];
	$dateout = $row["dateout"];
	echo "<tr><td style=\"padding:3px;\"><a href=movie.php?id=" . $movieid . ">" . $title .
	"</a></td> <td style=\"padding:3px;\">" . $dateout . "</td> <td style=\"padding:3px;\"><a href=return.php?id=" . $movieid . ">" . "Return</a></tr>";
	}
?>
